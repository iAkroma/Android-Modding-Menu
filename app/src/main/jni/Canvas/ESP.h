...
public:
    ...
    void DrawLine(Color color, float thickness, Vector2 start, Vector2 end, bool sh) {
        if (isValid()) {
            jclass canvasView = _env->GetObjectClass(_cvsView);
            jmethodID drawline = _env->GetMethodID(canvasView, "DrawLine",
                                                   "(Landroid/graphics/Canvas;IIIIFFFFFZ)V");
            _env->CallVoidMethod(_cvsView, drawline, _cvs, (int) color.a, (int) color.r,
                                 (int) color.g, (int) color.b,
                                 thickness,
                                 start.X, start.Y, end.X, end.Y, sh);
        }
    }

    void DrawText(Color color, const char *txt, Vector2 pos, float size, bool shd) {
        if (isValid()) {
            jclass canvasView = _env->GetObjectClass(_cvsView);
    ...
    void DrawImage(const char* img, int x, int y, int w, int h) {
        if (isValid()) {
            jclass canvasView = _env->GetObjectClass(_cvsView);
            jmethodID drawfilledcircle = _env->GetMethodID(canvasView, "DrawImage",
                                                           "(Landroid/graphics/Canvas;Ljava/lang/String;IIII)V");
            _env->CallVoidMethod(_cvsView, drawfilledcircle, _cvs, _env->NewStringUTF(img), (int) x, (int) y, (int) w, (int) h);
        }
    }
    ...
    void DrawCrosshair(Color clr, Vector2 center, float size = 20) {
        ...
        DrawLine(clr, 3, Vector2(x, center.Y), Vector2(x + size, center.Y), false);
        DrawLine(clr, 3, Vector2(center.X, y), Vector2(center.X, y + size), false);
    }
	void DrawBox(Color Color, float stroke, Rect rect) {
        ...
        DrawLine(Color, stroke, v1, v2, false);
        DrawLine(Color, stroke, v2, v3, false);
        DrawLine(Color, stroke, v3, v4, false);
        DrawLine(Color, stroke, v4, v1, false);
    }
	void DrawRect(Color Color, int width, float x1, float y1, float x2, float y2, int radius, bool sh) {
         if (isValid()) {
            jclass canvasView = _env->GetObjectClass(_cvsView);
            jmethodID gayp = _env->GetMethodID(canvasView, "DrawRect",
                                                           "(Landroid/graphics/Canvas;IIIIIFFFFIZ)V");
            _env->CallVoidMethod(_cvsView, gayp, _cvs, (int) Color.a, (int) Color.r,
                                 (int) Color.g, (int) Color.b, width, x1, y1, x2, y2, radius, sh);
        }
    }
	void DrawFilledRect(Color Color, float x1, float y1, float x2, float y2, int radius, bool sh) {
         if (isValid()) {
            jclass canvasView = _env->GetObjectClass(_cvsView);
            jmethodID gayp = _env->GetMethodID(canvasView, "DrawFilledRect",
                                                           "(Landroid/graphics/Canvas;IIIIFFFFIZ)V");
            _env->CallVoidMethod(_cvsView, gayp, _cvs, (int) Color.a, (int) Color.r,
                                 (int) Color.g, (int) Color.b, x1, y1, x2, y2, radius, sh);
        }
    }
};
